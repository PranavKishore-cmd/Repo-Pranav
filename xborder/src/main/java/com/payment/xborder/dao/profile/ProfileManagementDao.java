...
public class ProfileManagementDao {
   ...
   public List <ProfileManager> getAllProfiles() {
      return mongoTemplate.findAll(ProfileManager.class);
   }

   public ProfileManager getProfileDetails(String companyRefId) {
      Query query = new Query(Criteria.where("companyRefId").is(companyRefId));
      ProfileManager profileManager = mongoTemplate.findOne(query, ProfileManager.class);
